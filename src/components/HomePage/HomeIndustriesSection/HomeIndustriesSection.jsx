...
const HomeIndustriesSection = () => {
  ...
  const handleClick = (index) => {
    ...
    const clickedIndustryName = industryNameRef.current.querySelector(
      `.industry-name:nth-child(${index + 1})`
    );
  ...
  const handleMouseLeave = () => {
    intervalRef.current = setInterval(() => {
      setCurrentIndustry(
        (prevIndustry) => (prevIndustry + 1) % industries.length
      );
    }, 5000);
  };
  ...
  useEffect(() => {
    const industryName = industryNameRef.current.querySelector(
      `.industry-name:nth-child(1)`
    );
            ...
            {industries.map((industry, index) => (
              <div
                className={`industry-name ${
                  currentIndustry === index ? "active-industry-name" : ""
                }`}
                key={index}
                onClick={() => {
                  handleClick(index);
          ...
          {industries.map((industry, index) => (
            <div
              className={`industry-slide ${
                currentIndustry === index ? "active-industry" : ""
              }`}
              key={index}
            >
              <img
